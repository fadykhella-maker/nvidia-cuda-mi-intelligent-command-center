...
.stApp{{background:transparent;color:#f2f5f2}}[data-testid="stMainBlockContainer"],.main .block-container{{width:min(560px,calc(100vw - 36px))!important;max-width:560px!important;margin:11vh auto 0!important;padding:30px 38px 28px!important;position:relative;z-index:5;background:rgba(8,14,10,.93)!important;border:1px solid #36503c!important;border-radius:24px;box-shadow:0 25px 90px #000;backdrop-filter:blur(5px)}}
[data-testid="stMainBlockContainer"]:before,.main .block-container:before{{content:"";display:block;position:relative;z-index:3;width:140px;height:66px;margin:0 auto 14px;background:url('{logo_uri}') center/contain no-repeat}}
