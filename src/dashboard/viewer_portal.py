...
.stApp,[data-testid="stAppViewContainer"],[data-testid="stMain"]{{background:transparent!important;color:#f2f5f2}}[data-testid="stMainBlockContainer"],.main .block-container{{width:min(650px,calc(100vw - 36px))!important;max-width:650px!important;margin:15vh auto 0!important;padding:20px 34px 18px!important;position:relative;z-index:5;background:rgba(8,14,10,.94)!important;border:1px solid #36503c!important;border-radius:18px;box-shadow:0 25px 90px #000}}
...
.nv-world{{position:fixed;inset:0;z-index:-1;overflow:hidden;background:#010302}}.nv-world .world-map-image{{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center;display:block;opacity:.82;filter:saturate(.94) brightness(.7)}}.nv-world:after{{content:"";position:absolute;inset:0;z-index:1;background:linear-gradient(rgba(1,5,2,.06),rgba(1,4,2,.27));pointer-events:none}}
...
.login-card-surface{{position:fixed;z-index:0;left:50%;top:15vh;transform:translateX(-50%);width:min(650px,calc(100vw - 36px));height:410px;background:rgba(4,9,6,.84);border:1.5px solid rgba(255,255,255,.82);border-radius:16px;box-shadow:0 25px 90px #000;pointer-events:none}}
