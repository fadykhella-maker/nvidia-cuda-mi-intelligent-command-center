...
.stApp{{background:#010302;color:#f2f5f2}}.main .block-container{{max-width:560px;margin-top:12vh;padding:34px 38px 32px;position:relative;z-index:5;background:rgba(8,14,10,.97);border:1px solid #29402e;border-radius:24px;box-shadow:0 25px 90px #000}}
.main .block-container:before{{content:"";display:block;width:150px;height:74px;margin:0 auto 18px;background:url('{logo_uri}') center/contain no-repeat}}
.nv-world{{position:fixed;inset:0;z-index:0;overflow:hidden;background:radial-gradient(circle at 50% 30%,#102014,#040905 43%,#010201 80%)}}
.nv-world svg{{position:absolute;inset:10% 0 0;width:100%;height:85%;opacity:.8}}.nv-head{{position:absolute;top:26px;left:28px;display:flex;flex-direction:column;color:#76b900;font:14px ui-monospace,monospace;letter-spacing:1px}}.nv-head b{{font:700 22px Inter,sans-serif}}.nv-head span{{color:#68716a}}
.grid{{stroke:#17311e;stroke-width:1;fill:none}}.land{{fill:#07120a;stroke:#1d3823;stroke-width:1.5}}.route{{fill:none;stroke:#76b900;stroke-width:1.5;stroke-dasharray:8 10;animation:flow 8s linear infinite;filter:drop-shadow(0 0 4px #76b900)}}.route.alt{{stroke:#32a8ff;animation-duration:11s}}.node{{fill:#76b900;stroke:#d9ff9c;stroke-width:2;filter:drop-shadow(0 0 7px #76b900);animation:pulse 2s ease-in-out infinite alternate}}
...
@media(max-width:700px){{.main .block-container{{margin-top:8vh;padding:26px 22px}}.nv-world svg{{width:180%;left:-40%}}.nv-head span{{display:none}}}}
</style>
<div class="nv-world" aria-hidden="true"><div class="nv-head"><b>NVIDIA Accelerated Intelligence</b><span>Kaggle GPU · CUDA Engineering · Global Compute Fabric</span></div>
...
def require_viewer() -> dict[str, str]:
    ...
    st.markdown(
        """
        <style>
        .nv-world,.portal-note,[data-testid="stCheckbox"]{display:none!important}
